...
namespace fn_dag {
  ...
  */
   ...
   * 
   * The DAG calls the source generator to generate some data. That data is then propagated to
   * it's children and across the entire DAG. The DAG stops propagating across the children of
   * any node of the DAG if the previous parent returns nullptr. Otherwise, this runs either
   * multithreaded or single threaded and can be manually manipulated (i.e. skip the generator)
   * by turning it off and manually calling the *manual_pump* function.
  */
  ...
  private:
    ...
    dag_fanout_node<OriginType, IDType> m_children;   // The children of the source to propagate data across
    unordered_set<IDType> m_children_ids;             // An optimization: a quick O(1) set lookup of the children IDs
    const _dag_context &g_context;                    // The shared state across all of the children of this node.
    thread m_thread;                                  // Thread to run on if this DAG runs multi-threaded.

  public:
    /** Constructor of the DAG. Ideally this is created in the manager but you can create your own DAG if you'd like.
     * 
     * @param _id The ID of the DAG itself
     * @param _lsource The main generator (source) of the DAG. Where data originates
     * @param _context The context shared across the DAG
     * @param _startThread Whether or not to autostart calling the generator function
    */
    dag(IDType _id, 
    ...
    */
     ...
     * 
     * This is a useful function for testing. It *skips* the generator and pushes the test
     * source data across the DAGs children. 
     * 
    ...
    */
     ...
     * 
     * Since the generator already exists, this function recursively calls itself with the
     * parents nodes type and, in a strongly typed way, attaches a child to the parent to
     * process the data output by the parent node.
     * 
    ...
    */
     ...
     * 
     * This function can be called from the thread on a loop or called on a single thread.
     * This encapsulates a single pass across the DAG. 
    */
    void push_once() {
