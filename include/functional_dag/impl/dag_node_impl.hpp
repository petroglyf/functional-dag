...
namespace fn_dag {
   ...
   * 
   * This should not be used by users. It is simply a pure virtual class for internal 
   * nodes
  */
  ...
  public:
    ...
    /** Must provide a way to get an ID. Could be string or int or something efficient. */
  ...
  class _internal_dag_node : public _abstract_internal_dag_node<In, IDType> {
    friend class dag_fanout_node<In, IDType>;   // Friend classing fanout for the add_node search
    
  private:
    dag_node<In,Out> *m_node_hook;              // The function to run
    const IDType m_node_id;                     // The ID of the node
    dag_fanout_node<Out, IDType> *m_child;      // All of the children to provide our output data to.
    const fn_dag::_dag_context &g_context;      // A hook to the global context of this DAG.
  ...
  public:
     ...
     * @param _context The state variables for the DAG. All nodes share this information.
    */
     ...
     * 
     * This function simply encapsulates the process of calling update on the input data,
     * collecting the output data and propagating it to all of the children to be processed. 
     * 
