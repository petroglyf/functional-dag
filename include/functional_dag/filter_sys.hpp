...
namespace fn_dag {
  ...
  /** The DAG manager manages all of the dags created by the user. Think of it like a forest of trees. 
   * 
   * The dag manager is the main interfaces that users should interact with. You should
   * be able to create new root nodes (sources) and add leaves along the way (nodes). 
   */
  ...
  public:
    ...
    */
    bool manager_contains_id(IDType _id) {
      for(auto t = m_all_dags.cbegin();t != m_all_dags.cend();t++) {
        if((*t)->dag_contains(_id) || (*t)->get_id() == _id)
          return true;
      }
      return false;
    }
     ...
     * 
     * Setter for the print function to set the identation spaces between nodes when
     * printing the dags. 
     * 
    ...
    */
     ...
     * 
     * @param _is_single_threaded Whether or not new nodes and source nodes are on same node.
    */
     ...
     * @param _startImmediately Whether or not to begin generating data on a loop immediately.
     * 
     * @return Returns the created DAG if the user wants it. Otherwise, NULL if something fails.
    */
    ...
    */
     ...
     * 
     * This doesn't stop the nodes, this simply clears out the tracked DAGs so if you need to
     * stop the DAGs and maintain the pointers manually, feel free to do so. 
    */
    void clear() {
